...
def print_help():
    ...
    print "train.py [--help] [--nfd] [--nfatob] [--alpha] [--epochs] [batch_size] [--samples_per_batch] " \
          "[--save_every] [--learning_rate] [--beta_1] [--continue_train] [--log_dir]" \
    ...
    print "--learning_rate: The learning rate to train the models."
...
def model_creation(d, atob, params):
    ...
    opt = Adam(learning_rate=params.learning_rate, beta_1=params.beta_1)
...
def generators_creation(it_train, it_val, models, dout_size):
    ...
    # Workaround to make tensorflow work. When atob.predict is called the first time it calls tf.get_default_graph. This should be done on the main thread
...
if __name__ == '__main__':
    ...
    params = MyDict({
        ...
        'learning_rate': 2e-4,  # The learning rate to train the models
        ...
        'base_dir': 'minor',  # Directory that contains the data
    ...
    for opt, arg in opts:
        ...
        elif opt in ('--learning_rate', '--beta_1', '--rotation_range', '--height_shift_range',
    ...
    dopt = Adam(learning_rate=params.learning_rate, beta_1=params.beta_1)
    ...
    it_train = TwoImageIterator(train_dir,  is_a_binary=params.is_a_binary,
                                ...
                                is_b_binary=params.is_b_binary, batch_size=params.batch_size,
                                load_to_memory=params.load_to_memory, rotation_range=params.rotation_range,
                                height_shift_range=params.height_shift_range, width_shift_range=params.height_shift_range,
                                zoom_range=params.zoom_range, horizontal_flip=params.horizontal_flip,
                                vertical_flip=params.vertical_flip, target_size=(ts, ts))
    val_dir = os.path.join(params.base_dir, params.val_dir)
    it_val = TwoImageIterator(val_dir,  is_a_binary=params.is_a_binary, is_b_binary=params.is_b_binary, is_a_grayscale=params.is_a_grayscale, is_b_grayscale=params.is_b_grayscale,
                              batch_size=params.batch_size, load_to_memory=params.load_to_memory,  target_size=(ts, ts))
